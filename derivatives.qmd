...
In business management, derivatives can be used to optimize profit. Assume we have a function representing the profit of a company as a function of the price, $f(p)$. The derivative $f'(p)$ gives the rate of change of profit with respect to the price. If $f'(p) > 0$, producing more units will increase profit, if $f'(p) < 0$, producing more will decrease profit, and if $f'(xp) = 0$, we've reached a maximum or minimum point.
...
To find the price level that maximizes profits, we first obtain the derivative function $f'(p) = -2p + 20$, solve the equation $-2p + 20 = 0$ and find that the optimal price level $p = 10$ results in a profit of $-(10^2) + 20 * 10 =  100$.
